...
- **跨 CLI 外派才要算账。** 外派的正当理由只有一个：**换厂商**（交叉验证、独立第二意见、错误相关性更低）。为了"省钱"而把小任务外派给便宜模型，多半是净亏损。
...
**对基准测量的影响**：跨 CLI 的"模型对比"实际是"agent 对比"。做严格模型对比必须控制环境（干净的 `CODEX_HOME` / 停用 skills），否则结论无效。本项目的高难度基准 codex 臂即因此全部作废——详见维护者本地基准归档中的作废记录。
