...
| 任务性质 | 通道 | 为什么 |
|---|---|---|
| **纯文本**：分类 / 摘要 / 翻译 / 抽取 / 自包含问答（不需要读文件、不需要跑代码） | **aichat 或裸 API 直调**（⚠️ 仅按量用户或保额度场景——纯订阅用户走订阅内通道，别为省"已付过的 token"掏钱，见「成本的真正单位」），量大时**批量合并成一次调用** | 地板 ~几十 token，中低档模型在这里才真便宜；且输入干净——不受宿主 skill/AGENTS.md 污染 |
...
- **每次派发前先输出一行路由决策行**，固定格式：`[ai-cross] <任务> → <通道/模型> ｜ <档位> · thinking=<关/低/中/高> ｜ 理由：<一句话>`。两个旋钮与理由缺一不可——这一行是用户唯一能在执行前纠错的窗口。**单发模式（`-p`/`exec`）下用户看不到中途输出时，决策行必须原样回显在最终汇总里**（行为实测：不写明就会被省略）。
...
7. 汇总必含：**路由决策行（原样回显，见「路由公示」）**、谁干的、验证了什么、共识、分歧、**独有发现**、未验证项、**各路 token 用量与耗时**。
   ...
   - **注意固定上下文足迹**：每次外部 CLI 调用自带系统提示+工具定义的固定 input（实测 `codex exec` ≈20k；`claude -p` 12k/14k/31k，取决于 `--tools` 收得多窄）。**计费是双峰的**：冷调用付全额 fresh，热调用几乎全走缓存（TTL 约 5 分钟）→ 成串同类派发很便宜；零散一次性派发每次全额；批量合并的收益来自把冷调用摊薄成一次。
